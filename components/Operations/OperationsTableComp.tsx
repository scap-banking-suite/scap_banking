...
const OperationsTableComp = <T,>({ headers, data, renderRow }: TableProps<T>) => {
  return (
    <div className="overflow-x-auto scrollableCSS">
      <table className="min-w-full table-auto tablingOperation">
        <thead className="text-left ">
          <tr className="bg-operationBg text-tableText text-sm font-bold">
            {headers.map((header, index) => (
              <th key={index} className="py-4 px-4 border-b text-nowrap">
                {header?.content}
              </th>
            ))}
          </tr>
...
export default OperationsTableComp;
